...
from sqlalchemy.orm import sessionmaker, selectinload, aliased, subqueryload
...
from app.core.exceptions import NotFoundError, DBError
from app.models.models import OrderTask, Order, Worker, WorkType, SystemSettings, WorkerToTask, WorkerActivity, \
    SystemWeekends, order_task_dependencies
...
class BaseDBWork:
    ...
    @staticmethod
    def generate_subqueryload_chain(relationships: list):
        """
        Генерирует цепочку subqueryload для списка отношений.

        Args:
            relationships: Список отношений (например, [Order.tasks, OrderTask.prerequisites, ...])

        Returns:
            Цепочка subqueryload
        """
        # Начинаем с первого отношения
        chain = subqueryload(relationships[0])
        # Добавляем остальные отношения через .subqueryload()
        for rel in relationships[1:]:
            chain = chain.subqueryload(rel)
    ...
    async def get_objects(
            ...
            search: List = None,
            sort: List[Sort] = None,
            attr_for_load: Union[list, str] = None,
        ...
        if attr_for_load:
            ...
            query = query.options(self.generate_subqueryload_chain(relationships))
        if sort:
    ...
    async def update_obj(self, model, where: dict, for_set: dict):
    ...
    async def create_many_to_many(self, table, values: Union[dict, list]):
        try:
